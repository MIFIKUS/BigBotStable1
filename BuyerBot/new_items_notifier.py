...
items_list = get_items_list()['purple'] + get_items_list()['red']
...
while True:
    for server_id, server_name in servers_list.items():
        market_info, token = get_market_info(int(server_id), token)

        for i in market_info.items():
            if not items_list.get('purple').get(i[0]) and not items_list.get('red').get(i[0]) and not trash_list.get(i[0]):
