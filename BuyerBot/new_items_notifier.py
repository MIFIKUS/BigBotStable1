...
while True:
    for server_id, server_name in servers_list.items():
        ...
        for j in market_info:
            ...
            if not items_list.get(j['group_game_item_key']) and not trash_list.get(j['group_game_item_key']):
                if j['group_game_item_key'] not in items_to_research:
                    items_to_research.append(j['group_game_item_key'])
