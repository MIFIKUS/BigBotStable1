...
ctypes.windll.kernel32.VirtualProtectEx.argtypes = [HANDLE, LPVOID, ctypes.c_size_t, DWORD, LPVOID]
ctypes.windll.kernel32.VirtualProtectEx.restype = BOOL

ctypes.windll.kernel32.OpenProcess.restype = HANDLE
ctypes.windll.kernel32.VirtualAllocEx.restype = LPVOID


def bytes_from_short(value: int) -> bytes:
    """Возвращает byte представление short значения.

    Return byte representation of short value.
    """

    return struct.pack("<h", value)


def bytes_from_ushort(value: int) -> bytes:
    """Возвращает byte представление ushort значения.

    Return byte representation of ushort value.
    """
...
def bytes_from_int(value: int) -> bytes:
    """Возвращает byte представление int значения.

    Return byte representation of int value.
    """

    return struct.pack("<i", value)


def bytes_from_uint(value: int) -> bytes:
    """Возвращает byte представление uint значения.

    Return byte representation of uint value.
    """

    return struct.pack("<I", value)


def bytes_from_long(value: int) -> bytes:
    """Возвращает byte представление long значения.

    Return byte representation of long value.
    """

    return struct.pack("<l", value)


def bytes_from_ulong(value: int) -> bytes:
    """Возвращает byte представление ulong значения.

    Return byte representation of ulong value.
    """

    return struct.pack("<L", value)


def bytes_from_longlong(value: int) -> bytes:
    """Возвращает byte представление longlong значения.

    Return byte representation of longlong value.
    """

    return struct.pack("<q", value)


def bytes_from_ulonglong(value: int) -> bytes:
    """Возвращает byte представление ulonglong значения.

    Return byte representation of ulonglong value.
    """

    return struct.pack("<Q", value)


def bytes_from_float(value: float) -> bytes:
    """Возвращает byte представление float значения.

    Return byte representation of float value.
    """

    return struct.pack("<f", value)


def bytes_from_double(value: float) -> bytes:
    """Возвращает byte представление double значения.

    Return byte representation of double value.
    """
...
def table_memory(process: object, address: int, rows: int, row_length: int = 24) -> None:
    ...
    try:
        ...
        for row in range(rows):
            row_bytes = process.read_bytes(address + (row * row_length), row_length)
            print(hex(address + (row * row_length))[2:len(hex(address + (row * row_length)))].upper() + ' | ' + heximate_bytes(row_bytes))
...
def list_modules(process_handle: int) -> list:
    ...
    process_module_success = ctypes.windll.psapi.EnumProcessModulesEx(process_handle, ctypes.byref(modules), ctypes.sizeof(modules), ctypes.byref(ctypes.c_ulong()), LIST_MODULES_ALL)

    if process_module_success:
        modules = []
        modules = iter(module for module in modules if module)

        for module in modules:
            module_info = MODULE(process_handle)
            ctypes.windll.psapi.GetModuleInformation(process_handle, ctypes.c_void_p(module), ctypes.byref(module_info), ctypes.sizeof(module_info))
            modules.append(module_info.name)

        return modules


def list_memory_sections(process_handle: int, end_address: int) -> list:
    """Возвращает список (объекты MEMORY_BASIC_INFORMATION) со всеми секциями памяти процесса.

    Return list (object MEMORY_BASIC_INFORMATION) of all memory section of process."""

    return_list = []
    check_region = 0

    while check_region < end_address:
        memory_region_information = virtual_query_ex(process_handle, check_region)
        check_region = memory_region_information.BaseAddress + memory_region_information.RegionSize
        return_list.append(memory_region_information)
...
def set_memory_section_protection(process_handle: int, address: int, page_size: int, desired_protection: int) -> None:
    """Устанавливает защиту на участок памяти в данном процессе.

    Set protection to memory region in given proccess."""

    old_protection = ctypes.pointer(DWORD())
    ctypes.windll.kernel32.VirtualProtectEx(process_handle, address, page_size, desired_protection, old_protection)
...
class TrampolineHook:
    ...
    def insert_bytecode(self, bytecode: bytes) -> None:
